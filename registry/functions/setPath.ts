...
/** A non-empty set path that does not target an array's `length`. */
export type ValidSetPath<T, Path extends ObjectPath> =
...
/** The immutable result of assigning `Value` at a literal path. */
export type SetPathResult<
  T extends object,
...
>(
  ...
  path: Path & ValidSetPath<T, Path>,
  value: Value
): SetPathResult<T, Path, Value> => {
  ...
  segments.forEach((segment, index) => {
    if (index === segments.length - 1) {
      ...
      defineReplacementProperty(target, source, segment, value);
      return;
    }
  ...
  return root as SetPathResult<T, Path, Value>;
};
