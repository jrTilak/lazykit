...
## Props
<PropsTable methodName="debounce"/>
 ---

- **`function`**: (required) The function that you want to debounce, meaning it will delay its execution until after a specified delay time.
  
- **`delay`**: (required) The amount of time (in milliseconds) to wait before executing the debounced function after the last call.
