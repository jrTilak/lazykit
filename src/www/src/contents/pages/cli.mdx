...
- **Path to store helper functions**: Specify where you want to save the utility functions, e.g., `@/helpers`.
- **Path to store React hooks**: Indicate the directory where you want to keep your React hooks, e.g., `@/hooks`.
...
To view a list of all available methods, use the following command:

```bash
npx @jrtilak/lazykit@latest list
```